...
модели `emb://<folder>/text-search-doc/latest` (чанки) и `.../text-search-query/latest`
(запросы) — строго асимметрично, dim=256, батча НЕТ (по одному тексту, до 8 параллельных,
троттлинг 9 RPS). Порядок пересборки на новой машине:
