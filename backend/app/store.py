...
class Store:
    ...
    def _migrate_line_names(self):
        """Нормализация демо-линий: корректные отображаемые имена объектов +
        слияние ore-вариантов одной фабрики в одну линию (одна фабрика — одна
        схема). Нужна для уже существующих БД (деплой персистентный): линии из
        сырых plant-строк проектов заводит _migrate_project_lines, а тут им
        даются человекочитаемые имена. id стабильны (по ним привязано
        оборудование) — при переименовании меняем только name; при слиянии
        репойнтим проект, переносим line-scoped данные и удаляем лишнюю линию.
        Идемпотентно и только по конкретным известным демо-линиям — чужие
        линии и чужие переименования не затрагиваем."""
        # 1. слияние: НОФ медистые / «линия 1» -> одна линия НОФ (общая схема)
        nof = "НОФ · вкрапленные руды"
        for med in ("НОФ · линия 1 · Cu-порфир", "НОФ · медистые руды"):
            if not self._conn.execute("SELECT 1 FROM lines WHERE id=?", (med,)).fetchone():
                continue
            self._conn.execute("UPDATE projects SET plant=? WHERE plant=?", (nof, med))
