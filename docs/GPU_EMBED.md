...
1. Взять код и данные:
   ...
   cd hack_nornikil
   ```
   Скопировать с ноутбука папку `storage\kb` ЦЕЛИКОМ (~425 МБ: chunks.jsonl,
   meta.json, chroma\, files\, translations.json) в то же место клона —
   `hack_nornikil\storage\kb`. Процессы на ноуте уже остановлены, копия
   консистентна (кто копирует позже — убедитесь, что бэкенд/инжест не бегут).
